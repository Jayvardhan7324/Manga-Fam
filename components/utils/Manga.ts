    ...
    if (recents) {
      for (const item of recents) {
        ...
        if (found) {
          ...
          if (!item.readChapters.includes(readChapter)) item.readChapters.push(readChapter)
        }
      }
    }
...
export type { Manga, Relationship, Tag, ContentRating, PublicationDemographic, Status, RecentType }
