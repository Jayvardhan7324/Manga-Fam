...
const MangaCover: FC<{ manga: Manga, loading: boolean }> = ({ loading, manga }) => {
...
const UpdatedAt = ({ updatedDate, theme }: { updatedDate: Date, theme: "LIGHT" | "DARK" }) => {
  return (
    <div className={classname(
      ...
      <span>
          Last updated at: {`${updatedDate.getDate()}/${updatedDate.getMonth()}/${updatedDate.getFullYear()}`} (DD/MM/YY)
      </span>
    </div>
  )
}
  ...
  return (
    <>
      <Head>
        ...
        <meta name="description" content={description ? description.en : "No description available"} />
      ...
      <ModeContext.Consumer>
        {({ theme }) => (
          <div className="flex-shrink-1 flex flex-col flex-nowrap lg:pt-5">
            <NavLink {...{ manga }} />
            <section className="flex-grow-1 w-full">
              <div className="flex flex-col flex-nowrap xl:flex-row">
                <MangaCover {...{ loading, manga }}/>
                
                <div className="min-w-0 w-full flex flex-col flex-nowrap m-2 xl:ml-10">
                  <Title {...{ loading, title, altTitles }} />
                  <Author {...{ loading, author_name, publicationDemographic, status, contentRating }} />
