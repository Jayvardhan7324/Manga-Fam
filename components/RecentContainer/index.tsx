...
const sortRecents = (recents: any) => {
  ...
  for (const item of recents) {
    ...
    else if (lastRead < timeMap["Today"] && lastRead > timeMap["Yesterday"])
      manga_By_time["Yesterday"].push(item);
    else if (
      lastRead < timeMap["Yesterday"] &&
      lastRead > timeMap["2 Days Ago"]
    )
      manga_By_time["2 Days Ago"].push(item);
    else if (
      lastRead < timeMap["2 Days Ago"] &&
      lastRead > timeMap["3 Days Ago"]
    )
      manga_By_time["3 Days Ago"].push(item);
...
const RecentTimeHeading: FC<{ heading: string; mangas: Manga[] }> = ({
  heading,
  mangas,
}) => {
  return (
